									...
									<div class="messages-container margin-top-0">
										<div class="messages-headline">
											...
											if (!empty($_GET['messageid'])) {
												...
												$queryapp = "SELECT * from tblfeedback WHERE ADMINID = '$FEEDID' order by FEEDBACKID DESC";
												$resultapp = mysqli_query($con, $queryapp);
												$rowapp = mysqli_fetch_array($resultapp);
												$ADMINID = $rowapp['ADMINID'];

												$query = "UPDATE tblfeedback SET STATUS = 'Read' WHERE ADMINID = '$FEEDID' and SENTBY = '$FEEDID'";
												$result = mysqli_query($con, $query);

												//////APPLICANT DETAIL///////
												$queryapp = "SELECT * from tblusers WHERE USERID = '$ADMINID'";
												$resultapp = mysqli_query($con, $queryapp);
												$rowapp = mysqli_fetch_array($resultapp);
												$FNAME = $rowapp['FNAME'];
												$OTHERNAMES = $rowapp['ONAME'];
											?>
										...
										<div class="messages-container-inner">
											...
											<div class="dash-msg-inbox">
												<ul>
													...
													?>
													<?php
													$query = "SELECT * from tblfeedback where SENTBY !='$session_id' and APPLICANTID = '$session_id' ORDER BY FEEDBACKID DESC" or die(mysqli_error($con));
													...
													?>
														<li>
															<a href="?messageid=<?php echo $ADMINID ?>">
													...
													<?php }
													if ($count < 1) { ?>
														<p class="m-5 text-center text-muted"><i>No message yet...</i></p>
													<?php }
													?>
											...
											?>
												...
												<div class="dash-msg-content">
													...
													<?php
													$query = "SELECT * from tblfeedback where ADMINID = '$messageid' and APPLICANTID = '$session_id' ORDER BY FEEDBACKID DESC" or die(mysqli_error($con));
													$run = mysqli_query($con, $query);
													$count = 0;
													while ($row = mysqli_fetch_array($run)) {
														$count++;
													...
													?>
														...
														<?php } else { ?>
															...
															<div class="message-plunch">
																<div class="dash-msg-avatar">
																	<!-- <img src="../<?php echo $APPLICANTPHOTO ?>" alt=""> -->
																	<span class="mr-2 mb-2 d-inline-flex px-2 py-1 rounded theme-cl theme-bg-light text-warning">Admin:</span>
																</div>
																<div class="dash-msg-text">
																	<p><?php echo $FEEDBACK ?></p>
																</div>
												...
												<!-- Message Content -->
											<?php } else { ?>
												<p class="m-5 text-center text-muted"><i>Select Message to Display...</i></p>
											<?php } ?>
